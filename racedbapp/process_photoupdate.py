...
def index(request_date):
    events = get_events(request_date)
    numevents = len(events)
    if numevents == 0:
        logger.warning("No events found for {}".format(request_date))
    else:
...
def get_events(qsdate):
    ...
    if qsdate == "all":
        ...
        if hour in range(6):
            ...
            logger.info(
                "Auto checking events past month, past two years {}, "
                "anniversary day (batch {})".format(months[hour], hour)
            )
            past_two_years_events = list(
                Event.objects.filter(
                    date__lt=month_ago,
                    date__gte=two_years_ago,
                    date__month__in=months[hour],
    ...
    for i in alltags2add:
        try:
            ...
            for t in newtags["tags"]["tag"]:
                logger.info(
                    "New tag: event={} photo_id={} tag_content={} tag_id={}".format(
                        event.id, i[0], t["_content"], t["full_tag_id"]
                    )
                )
...
def get_member_assumption(event):
    ...
    # no_assumption_events = (813, 819, 1344, 1181, 1182, 1068, 1069, 1185, 1186)
