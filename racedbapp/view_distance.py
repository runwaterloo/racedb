...
def index(request, distance_slug):
    ...
    named_distance_record = namedtuple('ndr', ['female',
                                               'male',
                                               'masters_female',
                                               'masters_male'])
    distance_results = Result.objects.filter(event__distance=distance)
    records = []
    records += get_dist_records('Overall Male', distance_results, 'M', False, membership)
    records += get_dist_records('Overall Female', distance_results, 'F', False, membership)
    records += get_dist_records('Masters Male', distance_results, 'M', True, membership)
    records += get_dist_records('Masters Female', distance_results, 'F', True, membership)
...
def get_dist_records(place, distance_results, gender, masters, membership):
    ...
    if masters: 
        filtered_results = filtered_results.filter(Q(category__ismasters=True) | Q(age__gte=40))
...
def get_resultfilter(distance_slug, topx_filter):
    ...
    for f in ['Female', 'Male', 'Masters', 'F-Masters', 'M-Masters']:
        if f != topx_filter:
           choices.append(namedchoice(f, '/distance/{}/?filter={}'.format(distance_slug, f)))
