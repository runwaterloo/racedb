...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in city_count.keys():
            sorted_city_count = sorted(city_count[i].items(), key=operator.itemgetter(0))
            sorted_city_count = sorted(sorted_city_count, key=operator.itemgetter(1), reverse=True)
            print()
            print(i.upper())
