...
def index(request, race_slug, distance_slug):
    ...
    if distance_slug == 'combined':
        ...
        rawresults = Result.objects.select_related().filter(event__race__in=races, event__distance=distance)
    ...
    records, team_records, hill_records = view_shared.getracerecords(race, distance, division_choice)
    ...
    for result in results:
        ...
        final_results.append(namedresult(count, guntime,
                             result.athlete, year,
                             result.category,
                             result.city, extra, age,
                             result_race_slug, member))
        count += 1
    ...
    if race.slug == 'endurrun' and division_choice != 'Ultimate':
        choices = [namedchoice('', '/race/{}/{}/?division={}'.format(race_slug, distance_slug, division_choice)),
                   namedchoice('Female', '/race/{}/{}/?division={}&filter=Female'.format(race_slug, distance_slug, division_choice)),
                   namedchoice('Male', '/race/{}/{}/?division={}&filter=Male'.format(race_slug, distance_slug, division_choice))]
        if 'Masters Female' in record_divs and 'Masters Male' in record_divs:
            choices.append(namedchoice('Masters', '/race/{}/{}/?division={}&filter=Masters'.format(race_slug, distance_slug, division_choice)))
        if 'Masters Female' in record_divs:
            choices.append(namedchoice('F-Masters', '/race/{}/{}/?division={}&filter=F-Masters'.format(race_slug, distance_slug, division_choice)))
        if 'Masters Male' in record_divs:
            choices.append(namedchoice('M-Masters', '/race/{}/{}/?division={}&filter=M-Masters'.format(race_slug, distance_slug, division_choice)))
    else:
        choices = [namedchoice('', '/race/{}/{}'.format(race_slug, distance_slug)),
                   namedchoice('Female', '/race/{}/{}/?filter=Female'.format(race_slug, distance_slug)),
                   namedchoice('Male', '/race/{}/{}/?filter=Male'.format(race_slug, distance_slug))]
        if 'Masters Female' in record_divs and 'Masters Male' in record_divs:
            choices.append(namedchoice('Masters', '/race/{}/{}/?filter=Masters'.format(race_slug, distance_slug)))
        if 'Masters Female' in record_divs:
            choices.append(namedchoice('F-Masters', '/race/{}/{}/?filter=F-Masters'.format(race_slug, distance_slug)))
        if 'Masters Male' in record_divs:
            choices.append(namedchoice('M-Masters', '/race/{}/{}/?filter=M-Masters'.format(race_slug, distance_slug)))
...
def get_hill_results():
    named_hill_result = namedtuple('nhr', ['place',
                                           'female_time',
                                           'female_result',
                                           'male_time',
                                           'male_result'])
    results = Result.objects.filter(event__race__slug='baden-road-races',
                                    event__distance__slug='7-mi')
    hill_results = []
    ...
    for i in range(1, 11):
        ...
        hill_results.append(named_hill_result(i,
                                              female_prime,
